...
| 檔案 | 版本 | 大小 | SHA256 |
|---|---|---|---|
| [`AwayTerminal-Setup-1.0.21.exe`](./AwayTerminal-Setup-1.0.21.exe) | 1.0.21 | 61.8 MB | `1493482CCB459D4325E58F246B062B34937518C4415CB98B2981ABD2D8B6968D` |
...
- **Telegram 遠端自動推播不再卡住視窗**：分頁忙完自動推播時會凍結畫面約 1.5 秒，
  而且每次都退回品質較差的備援文字。改到背景執行緒後，推播內容改用終端機實際渲染的畫面，
  「只推新輸出」的比對也更準確
- **關閉程式時的當機修正**：勾了「Claude Code 離開前更新 CLAUDE.md」後，
  在等待期間按視窗右上角 X 會讓程式當掉
- **「Claude Code 離開前更新 CLAUDE.md」恢復正常**：1.0.18 把 ClaudeCode 改成自訂連線後，
  這個選項就一直是灰色的、實際上沒有作用
- **Telnet 連線改為背景進行**：連線到不通的主機不再讓整個視窗卡住約 20 秒
  （斷線自動重連時每次重試都會卡一次）；連不上會直接在畫面顯示錯誤訊息
- **修正連線資源沒有釋放**：斷線重連時舊連線的資源不會被回收，長時間掛著會逐次累積。
  另外拔掉 USB 序列埠裝置現在也會正確觸發自動重連
- **記錄檔中文不再出現亂碼**：中文字剛好被資料切斷時會變成「�」（畫面顯示不受影響）
...
1. 下載並執行 `AwayTerminal-Setup-1.0.21.exe`。
2. 若跳出「Windows 已保護你的電腦」→ 點 **其他資訊 → 仍要執行**。
3. UAC 出現按 **是**（安裝需系統管理員權限：安裝程式本體，必要時安裝 .NET 9 Desktop Runtime
   與 WebView2 執行環境）。
4. 安裝完成後，程式本體不會再被 Windows 擋。

> 安裝過程**不會**把憑證匯入系統的「受信任的根」（1.0.11 以前的版本會，1.0.12 起已移除）。
> 若希望 UAC 顯示發行者名稱，可自行執行安裝目錄下的 `trust-publisher.ps1`，
> 它只寫入你自己的憑證存放區、不需系統管理員權限。憑證指紋：
> `7B11D2A5062A07C5BDD440A8C6B34FD3D7E5719D`
